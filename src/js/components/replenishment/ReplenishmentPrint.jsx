...
import { REPLENISHMENT_PRINT_DATA } from 'api/urls';
import {
...
const ItemRows = ({ item, index }) => {
  ...
  return lines.map((line, j) => (
    ...
    <tr className="prop" style={{ backgroundColor }} key={`${item.id}-${j}`}>
      ...
      {j === 0 && (
        <>
          <td className="center" width="1%" rowSpan={rowspan}>{item.destinationBin}</td>
          <td className="center" width="1%" rowSpan={rowspan}>{item.quantity}</td>
        </>
      )}
    ...
    <table className="items-table fs-repeat-header" border="0">
      <thead style={{ display: 'table-row-group' }}>
        ...
        <tr className="theader">
          ...
          <th className="center">{data.columns[8].title}</th>
          <th className="center" style={{ minWidth: '100px' }}>{data.columns[9].title}</th>
        </tr>
      </thead>
      <tbody>
        {!group.items.length && (
...
const ReplenishmentPrint = ({ match }) => {
  ...
  useEffect(() => {
    apiClient.get(REPLENISHMENT_PRINT_DATA(orderId))
        ...
        >
          ...
          {zone.groups.map((group) => (
            <GroupTable
              ...
              data={data}
              group={group}
              pageBreakAfter={enablePageBreak && !zone.showName ? 'always' : 'avoid'}
            />
          ))}
        </div>
      ))}
...
export default ReplenishmentPrint;

ReplenishmentPrint.propTypes = {
