...
const PreferenceTypeList = () => {
  ...
  const {
    ...
    errorMessageId: 'react.preferenceType.error.preferenceTypeList.label',
  ...
  const columns = useMemo(() => [
    {
      ...
      Cell: (row) => (
        <TableCell
          {...row}
          link={PREFERENCE_TYPE_URL.edit(row.original.id)}
        />
      ),
    },
    {
  ...
  return (
    <PageWrapper>
      <HeaderWrapper>
        <ListTitle label={{
          id: 'react.preferenceType.header.label',
          defaultMessage: 'Preference Types List',
        }}
        />
        <HeaderButtonsWrapper>
          {isUserAdmin && (
            <Button
              label="react.preferenceType.createPreferenceType.label"
              defaultLabel="Create Preference Type"
              onClick={() => {
                window.location = PREFERENCE_TYPE_URL.create();
              }}
            />
          )}
        </HeaderButtonsWrapper>
      ...
      <ListTableWrapper>
        <DataTable
          ...
          noDataText="No preference types match the given criteria"
          footerComponent={() => (
