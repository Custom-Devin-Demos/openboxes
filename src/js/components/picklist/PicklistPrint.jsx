...
import { PICKLIST_PRINT_DATA } from 'api/urls';
import {
      ...
      {j === 0 && (
        <>
          ...
          <td className="center" rowSpan={rowspan}>
            ...
            <div className={item.status}>{item.productCode}</div>
          </td>
          <td className="center" rowSpan={rowspan}>
            {item.substitutedParent && (
              <div className="canceled">{item.substitutedParent.productName}</div>
            )}
            <div className={item.status}>{item.productName}</div>
          </td>
          <td className="center" rowSpan={rowspan}>
            {item.parentChanged && (
    ...
    <table className="items-table fs-repeat-header" border="0">
      <thead style={{ display: 'table-row-group' }}>
        ...
        <tr className="theader">
          ...
          <th className="center border-right">{data.columns[3].title}</th>
...
const PicklistPrint = ({ match }) => {
  ...
  useEffect(() => {
    ...
    apiClient.get(PICKLIST_PRINT_DATA(requisitionId), { params: { sorted } })
